...
extension UIImage {
    ...
    class func animatedImageWithSource(_ source: CGImageSource) -> UIImage? {
        ...
        for cnt in 0..<count {
            if let image = CGImageSourceCreateImageAtIndex(source, cnt, nil) {
                images.append(image)
            }

            let delaySeconds = UIImage.delayForImageAtIndex(Int(cnt),
                source: source)
        ...
        var frameCount: Int
        for cnt in 0..<count {
            frame = UIImage(cgImage: images[Int(cnt)])
            frameCount = Int(delays[Int(cnt)] / gcd)
